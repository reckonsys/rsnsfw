...
def predict_nsfw(image_path):
    with tf.Session() as sess:
        ...
        probabilities, class_index = sess.run(
            [probabilities_op, class_index_op],
            feed_dict={inputs: [image_data] * _BATCH_SIZE})
        probabilities_dict = {
            _LABEL_MAP.get(i): l for i, l in enumerate(probabilities[0])}
        pre_label = _LABEL_MAP.get(class_index[0])
        return {"class": pre_label, "probability": probabilities_dict}


if __name__ == '__main__':
    argv = sys.argv
    if(len(argv) < 2):
        print("usage: python rsnsfw.py <image_path>")
    image_path = argv[1]
    print()
